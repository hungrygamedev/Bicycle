Shader "Toon/Lit Tri Planar" {
	...
	_Scale("Top Scale", Range(-2,2)) = 1
		...
		_RimColor2("Rim Color Side/Bottom", Color) = (0.5,0.5,0.5,1)
		 _Test("Test", Range(0,2)) = 0.0
			_Offset("Test", Range(0,2)) = 0.0
			_Extra("Test", Range(0,5)) = 0.0
	}
...
#pragma lighting ToonRamp 
	...
	float TB_OFFSET_X, TB_OFFSET_Z, TB_OFFSET_Y, TB_FARCLIP, TB_SCALE;
	sampler2D TB_DEPTH;
	float _Test, _Offset, _Extra;
	...
	void surf(Input IN, inout SurfaceOutput o) {
		...
		float2 uv = (IN.worldPos.xz - float2(TB_OFFSET_X, TB_OFFSET_Z)) / TB_SCALE;
		// Albedo comes from a texture tinted by color
		float c = tex2D(TB_DEPTH, uv).r;// *_Color;

		fixed4 cr = tex2D(_MainTex, IN.uv_MainTex);

		c *= TB_FARCLIP;
		c = IN.worldPos.y - c;
		c = TB_OFFSET_Y - c;

		float depth = (smoothstep(_Test, _Test + _Offset, c + _Extra));

		// final albedo color
		o.Albedo = topTextureResult + sideTextureResult + topTextureEdgeResult;// +depth;
		o.Albedo *= _Color;
