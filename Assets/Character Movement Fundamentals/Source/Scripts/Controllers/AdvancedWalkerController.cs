...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller
	{
		...
		public Vector3 savedVelocity2 = Vector3.zero;
		...
		protected virtual Vector3 CalculateMovementDirection()
		{
			...
			else
			{
				...
				{
					if (_PlayerController.speed > 10)
					{
						_velocity += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized *
						             (characterInput.GetHorizontalMovementInput()/5f);
						Debug.Log("AddV 1");
					}
					else
					{
				...
				else if(characterInput.GetHorizontalMovementInput()>0.5f || characterInput.GetHorizontalMovementInput()<-0.5f)
				{
					if (_PlayerController.speed > 10)
					{
						_velocity += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized *
					...
					_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized *
					             Mathf.Abs(characterInput.GetVerticalMovementInput());
				}*/
				
				_velocity += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized *
				             (_PlayerController.axisHToMove);
				if (_PlayerController.speed < _PlayerController.moveSpeed / 2f)
				{
					_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized *
					             Mathf.Abs(_PlayerController.axisVToMove);
				}
				else
				{
					_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized;
		...
		protected virtual Vector3 CalculateMovementVelocity()
		{
			...
			{
				lastDirection = _velocity;
			}
			if (_velocity == Vector3.zero && _PlayerController.speed > 0)
			{
				_velocity = lastDirection;
				_velocity *= _PlayerController.speed;
			}
			else
			{
				_velocity *= _PlayerController.speed;
