...
namespace CMF
{
	...
	public class TurnTowardControllerVelocity : MonoBehaviour {
		...
		void LateUpdate () {
			if (_CKI.GetHorizontalMovementInput() > 0.5f || _CKI.GetHorizontalMovementInput() < -0.5f)
			{
				...
				//currentYRotation = Mathf.Clamp(currentYRotation, -lowerVerticalLimit, lowerVerticalLimit);
				tr.localRotation = Quaternion.Euler(0f, currentYRotation, 0f);
			}else
			if (_CKI.GetVerticalMovementInput() > 0)
			{
