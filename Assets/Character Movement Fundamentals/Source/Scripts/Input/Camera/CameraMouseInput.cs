...
namespace CMF
{
    ...
    public class CameraMouseInput : CameraInput
    {
        ...
        private void Start()
        {
            _FixedTouchField = UIController.instance.touchField;
        }

        public override float GetHorizontalCameraInputw()
        {
            //Get raw mouse input;
            float _input = 0;
            if (useTouch)
            {
                _input = _FixedTouchField.TouchDist.x;
            }
            else
            {
        ...
        public override float GetVerticalCameraInput()
        {
           ...
           if (useTouch)
           {
               _input = _FixedTouchField.TouchDist.y;
           }
           else
           {
